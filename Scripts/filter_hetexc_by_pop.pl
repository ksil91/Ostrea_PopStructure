...
foreach my $pop (sort keys %pops) {
	...
	while(<HWEI>) {
		last if $_ =~ /^\s/;
		chomp;
		my ($locus, $pos, $obs, $exp, $chisq, $pvalue, $hetdef, $hetexc) = split;
		$exclude_count{"$locus-$pos"}++ if $hetexc < $hwe;
...
open(HWEO, ">", 'exclude.hetexc') or die $!;
...
my $output = `vcftools --vcf $vcffile --exclude-positions exclude.hetexc --recode --recode-INFO-all --out $outfile 2>&1`;
my $filt_output = `vcftools --vcf $vcffile --positions exclude.hetexc --hardy --out filtered 2>&1`;

print "Outputting results of HWE test for filtered loci to 'filtered.hetexc'\n";
