...
```{r}
## Get gps coordinates.
pop2gps <- read.table(file = "../Making_Files/All_Pop2Int_Loc.txt", header=T)
...
Write file for v2
```{r}
write.table(diffs.v2, paste(suf,"v2.diffs",sep=""), 
            col.names = FALSE, row.names = FALSE, quote = FALSE)
...
suf <- "EEMS/Allall-m70x62-mac4-xOR1"
geno <- Alla.xOR@tab
...
## Plotting
```{r}
eems.plots(mcmcpath = "EEMS/All-c80-m70x62-mac4-xOR1_results/Allall-m70x62mac4-xOR1-SkI-nD200-ch1/", plotpath = "EEMS/All-c80-m70x62-mac4-xOR1_results/Allall-m70x62mac4-xOR1-SkI-nD200-ch1-plots",longlat = T,add.grid=F,add.outline = T,add.demes = T,projection.in = "+proj=longlat +datum=WGS84",projection.out = "+proj=merc +datum=WGS84",add.map = T,add.abline = T, add.r.squared = T)
```